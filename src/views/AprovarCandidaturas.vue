<template>
  <div id="forumContent" class="d-flex justify-content-center">
    <b-col id="contentContainer" cols="12">
      <div id="forumTitle">
        <b-col cols="12">
          <h3 class="TitlePage">Aprovar Candidaturas</h3>
        </b-col>
      </div>
      <div>
        <b-col id="listCards" md="12">
          <div v-if="getPorposals.length > 0" class="mt-3" id="cardsDisplay">
            <div id="forumContent" class="d-flex justify-content-center">
                ...
                <tableTrCandidaturas v-for="prop in getPorposals" :key="prop.id_proposta" :tableTr="prop" />
                </b-table-simple>
            </div>
          </div>
...
<script>
import tableTrCandidaturas from "../components/aprovarCandTableData.vue";
...
export default {
  ...
  computed: {
    getPorposals() {
    try{
        console.log(this.$store.getters.getProposals)
